...
def parse_response_data(response_text: str) -> list:
    ...
    for line in lines[start_index:]:
        if line.strip() and not line.startswith('#7777END'):
            ...
            parsed_row = parse_columns(columns)
            if parsed_row:
...
def parse_columns(columns: list) -> tuple:
    try:
        ...
        return (stn_id, lon, lat, stn_sp, ht, ht_pa, ht_ta, ht_wd, ht_rn, stn_ad, stn_ko, stn_en, fct_id, law_id, basin)
...
def stn_inf_to_s3(data_interval_end: pendulum.datetime, **kwargs) -> None:
    ...
    if data:
        ...
        year, month, day = data_interval_end_kst.strftime('%Y'), data_interval_end_kst.strftime('%m'), data_interval_end_kst.strftime('%d')
...
def stn_inf_to_redshift(data_interval_end: pendulum.datetime, **kwargs) -> None:

...
def parse_response_data(response_text: str, column_len: int) -> list:
    if "#START7777" not in response_text or "#7777END" not in response_text:
        logging.error("ERROR : 데이터 수신 실패")
    ...
    for line in lines[start_index:]:
        if line.strip() and not line.startswith('#7777END'):
            columns = parse_columns(line.split(','), column_len)
            if columns:
                data.append(columns)
    
    return data

def parse_columns(columns: list, column_len: int) -> tuple:
    ...
    try:
        ...
        wf = columns[9]
        rn_st = columns[10]
        return (reg_id, tm_st, tm_ed, mod, stn, c, sky, pre, conf, wf, rn_st)
...
def save_to_s3(data: list, s3_key: str) -> None:
    ...
    try:
        ...
        logging.info(f"S3 저장 성공: {s3_key}")
...
def fct_afs_wl_to_s3(data_interval_end: pendulum.datetime, **kwargs) -> None:
    ...
    data = parse_response_data(response_text, column_len=12)

    if data:
        year = data[0][1].strftime('%Y')
        month = data[0][1].strftime('%m')
        day = data[0][1].strftime('%d')
        hour = data[0][1].strftime('%H')
        formatted_date = data[0][1].strftime('%Y_%m_%d_%H')
